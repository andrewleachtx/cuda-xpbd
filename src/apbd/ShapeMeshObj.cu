...
Eigen::Matrix<float, 6, 1> ShapeMeshObj::computeInertia(
    ...
    if (T0 == 0.0f) {
        throw std::runtime_error(
            "V0 is zero, will cause div by zero in "
            "ShapeMeshObj::computeInertia");
    }
    ...
    if (es.info() != Eigen::Success) {
        throw std::runtime_error(
            "Eigenvalue decomposition failed in ShapeMeshObj::computeInertia");
    }
...
cdata_t ShapeMeshObj::narrowphaseGround(
    ...
    /*
        Because narrowphase ground iterates over each vertex
    */
...
void ShapeMeshObj::readOBJ(const std::string &filename,
    ...
    if (!file.is_open()) {
        throw std::runtime_error("Couldn't open OBJ file in readOBJ: " +
                                 filename);
    }
    ...
    while (std::getline(file, line)) {
        ...
        // f_vt (f i/j i/j i/j)
        {
            int i1, i2, i3;
            int j1, j2, j3;
            int matches = sscanf(line.c_str(), "f %d/%d %d/%d %d/%d", &i1, &j1,
                                 &i2, &j2, &i3, &j3);
            if (matches == 6) {
                faces.emplace_back(i1, i2, i3);
                continue;
        ...
        // f_vn (i//n i//n i//n)
        {
            int i1, i2, i3;
            int n1, n2, n3;
            int matches = sscanf(line.c_str(), "f %d//%d %d//%d %d//%d", &i1,
                                 &n1, &i2, &n2, &i3, &n3);
            if (matches == 6) {
                faces.emplace_back(i1, i2, i3);
                continue;
    ...
    // FIXME: Not sure if it really matters whether we are in 1-based or
    // 0-based, can mess with this later
...
void ShapeMeshObj::VolumeIntegration(
    ...
    for (int i = 0; i < numFaces; i++) {
        ...
        /*
            Gets weird here. MATLAB does A = A + 1, B = B + 1, etc... but that
           is for 1-based
        ...
        */
        ...
        float P1 = 0, Pa = 0, Paa = 0, Paaa = 0, Pb = 0, Pbb = 0, Pbbb = 0,
              Pab = 0, Paab = 0, Pabb = 0;
        ...
        float Fbb = k1 * Pbb;
        float Fcc =
            k3 * ((N_A * N_A) * Paa + 2 * N_A * N_B * Pab + (N_B * N_B) * Pbb +
        ...
        float Fccc =
            -k4 *
            ((N_A * N_A * N_A) * Paaa + 3 * N_A * N_A * N_B * Paab +
             3 * N_A * N_B * N_B * Pabb + (N_B * N_B * N_B) * Pbbb +
             3 * w *
                 ((N_A * N_A) * Paa + 2 * N_A * N_B * Pab + (N_B * N_B) * Pbb) +
             w * w * (3 * (N_A * Pa + N_B * Pb) + w * P1));

        float Faab = k1 * Paab;
        float Fbbc = -k2 * (N_A * Pabb + N_B * Pbbb + w * Pbb);
        float Fcca = k3 * ((N_A * N_A) * Paaa + 2 * N_A * N_B * Paab +
                           (N_B * N_B) * Pabb +
