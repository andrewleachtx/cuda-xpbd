...
# valgrind --tool=memcheck --leak-check=full --track-origins=yes --show-leak-kinds=all ./build/$version/tests/performance -m $m -s $s -t $t
   cuda-gdb --args ./build/$version/tests/performance -m $m -s $s -t $t
fi