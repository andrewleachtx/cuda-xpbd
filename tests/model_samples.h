...
switch (modelID) {
        ...
        case 441: {
            ...
            for (size_t i = 0; i < 4; i++) {
                bodies[i] = apbd::Body(apbd::BodyRigid(apbd::ShapeCuboid{sides},
                                                       density, true, mu));
                Eigen::Matrix4f E = Eigen::Matrix4f::Identity();
                Eigen::Vector3f pos = Eigen::Vector3f(12, 0, (i + 0.5) * w);
                E.block<3, 1>(0, 3) = pos;
                bodies[i].setInitTransform(E);
            }
            ...
            bodies[5] = apbd::Body(apbd::BodyRigid(apbd::ShapeCuboid{Eigen::Vector3f(12, 4, 0.5f)},
                                            density, true, mu));
            E = Eigen::Matrix4f::Identity();
            pos = Eigen::Vector3f(-1.0f, 0.0f, 6.0f + 0.25f);
            E.block<3, 1>(0, 3) = pos;
            ...
            bodies[6] = apbd::Body(apbd::BodyRigid(apbd::ShapeCuboid{Eigen::Vector3f(w, w, w)},
                                            density, true, mu));
            E = Eigen::Matrix4f::Identity();
            pos = Eigen::Vector3f(-5.0f, 0.0f, 6.5f + 0.5f * w);
            E.block<3, 1>(0, 3) = pos;
        ...
        case 442: {
            ...
            bodies[10] = apbd::Body(apbd::BodyRigid(apbd::ShapeCuboid{sides},
                                                    density, true, mu));
            Eigen::Matrix4f E = Eigen::Matrix4f::Identity();
            Eigen::Vector3f pos = Eigen::Vector3f(-8.0f, 0.0f, 0.5f);
            E.block<3, 1>(0, 3) = pos;
